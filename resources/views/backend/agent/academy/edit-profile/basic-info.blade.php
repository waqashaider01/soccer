<div class="row">
    <div class="col-md-4">
        <form method="POST" action="{{ route('agent.basic-info-save') }}">
            @csrf
            <div class="mb-3">
                <label for="academy_name" class="form-label">Academy's Name *</label>
                <input class="form-control" type="text" name="academy_name" id="academy_name"
                    value="{{ $agent->academy_name ?? '' }}" placeholder="Enter academy name">
            </div>
            <div class="mb-3">
                <label for="position_at_academy" class="form-label">Position At Academy *</label>
                <select id="position_at_academy" class="form-select" name="position_at_academy">
                    <option value="" selected disabled>Select position</option>
                    <option value="owner" @if ($agent->position_at_academy == 'owner') selected @endif>Owner</option>
                    <option value="representative" @if ($agent->position_at_academy == 'representative') selected @endif>Representative</option>
                </select>
            </div>

            <div class="mb-3">
                <label for="academy_nationality" class="form-label">Academy Nationality *</label>
                <input type="text" class="form-control" id="academy_nationality" name="academy_nationality"
                    value="{{ $agent->academy_nationality }}" placeholder="e.g Pakistan">
            </div>
