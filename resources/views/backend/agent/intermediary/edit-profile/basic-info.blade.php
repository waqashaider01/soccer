<div class="row">
    <div class="col-md-4">
        <form method="POST" action="{{ route('agent.basic-info-save') }}">
            @csrf
            <div class="mb-3">
                <label for="agency_name" class="form-label">Agency's Name *</label>
                <input class="form-control" type="text" name="agency_name" id="agency_name"
                    value="{{ $agent->agency_name ?? '' }}" placeholder="Enter agency name">
            </div>
            <div class="mb-3">
                <label for="position_at_agency" class="form-label">Position At Agency *</label>
                <select id="position_at_agency" class="form-select" name="position_at_agency">
                    <option value="" selected disabled>Select position</option>
                    <option value="owner" @if ($agent->position_at_agency == 'owner') selected @endif>Owner</option>
                    <option value="representative" @if ($agent->position_at_agency == 'representative') selected @endif>Representative</option>
                </select>
            </div>

            <div class="mb-3">
                <label for="intermediary_nationality" class="form-label">Intermediary Nationality *</label>
                <input type="text" class="form-control" id="intermediary_nationality" name="intermediary_nationality"
                    value="{{ $agent->intermediary_nationality }}" placeholder="e.g Pakistan">
            </div>
