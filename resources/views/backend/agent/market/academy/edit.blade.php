...
@section('content')
<div class="container">
    <form method="POST" action="{{ route('academy.market-post-update') }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="mb-3">
                    <label for="academy_name" class="form-label">Academy Name *</label>
                    <input type="text" class="form-control" id="academy_name" name="academy_name"
                        value="{{ old('academy_name', $agent->academy_name) }}" placeholder="Enter academy name">
                </div>
                @error('academy_name')
                <div class="text-danger mb-3">{{ $message }}</div>
                @enderror
            ...
            <div class="col-md-6">
                ...
                <div class="mb-3">
                    ...
                    <select id="player_position" class="form-select player_position_multiple" name="player_position[]" multiple="multiple">
                        <option value="striker" @if (in_array('striker', $player_position)) selected @endif>Striker</option>
                        <option value="second-striker" @if (in_array('second-striker', $player_position)) selected @endif>Second Striker
                        </option>
                        <option value="right-forward" @if (in_array('right-forward', $player_position)) selected @endif>Right Forward
                        </option>
                        <option value="left-forward" @if (in_array('left-forward', $player_position)) selected @endif>Left Forward
                        </option>
                        <option value="attacking-midfielder" @if (in_array('attacking-midfielder', $player_position)) selected
                            @endif>Attacking
                            Midfielder</option>
                        <option value="central-midfielder" @if (in_array('central-midfielder', $player_position)) selected @endif>Central
                            Midfielder</option>
                        <option value="right-midfielder" @if (in_array('right-midfielde', $player_position)) selected @endif>Right
                            Midfielder
                        </option>
                        <option value="left-midfielder" @if (in_array('left-midfielder', $player_position)) selected @endif>Left
                            Midfielder
                        </option>
                        <option value="defensive-midfielder" @if (in_array('defensive-midfielder', $player_position)) selected
                            @endif>Defensive
                            Midfielder</option>
                        <option value="right-fullback" @if (in_array('right-fullback', $player_position)) selected @endif>Right Fullback
                        </option>
                        <option value="left-fullback" @if (in_array('left-fullback', $player_position)) selected @endif>Left Fullback
                        </option>
                        <option value="central-defender" @if (in_array('central-defender', $player_position)) selected @endif>Central
                            Defender</option>
                        <option value="right-wing-back" @if (in_array('right-wing-back', $player_position)) selected @endif>Right Wing
                            Back
                        </option>
                        <option value="left-wing-back" @if (in_array('left-wing-back', $player_position)) selected @endif>Left Wing Back
                        </option>
                        <option value="goalkeeper" @if (in_array('goalkeeper', $player_position)) selected @endif>Goalkeeper</option>
                    </select>
                </div>
