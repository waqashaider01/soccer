        ...
        <form method="POST" action="{{ route('agent.basic-info-save') }}">
            ...
            <div class="mb-3">
                ...
                <select id="position_at_organization" class="form-select" name="position_at_organization">
                    ...
                    <option value="owner" @if ($agent->position_at_organization == 'owner') selected @endif>Owner
                    </option>
                    <option value="representative" @if ($agent->position_at_organization == 'representative') selected
                        @endif>Representative</option>
                </select>
            </div>

            <div class="mb-3">
                <label for="coach_nationality" class="form-label">Coach Nationality *</label>
                <input type="text" class="form-control" id="coach_nationality" name="coach_nationality"
                    value="{{ $agent->coach_nationality }}" placeholder="e.g Pakistan">
            </div>
            ...
            <div class="mb-3">
                ...
                <select id="countries_of_operation" class="form-select chosen-select-country" multiple="multiple"
                    ...
                    <option value="{{ $country->name }}" @if (in_array($country->name,
                        json_decode($agent->countries_of_operation) ?? [])) selected @endif>{{ $country->name }}
                    </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                <label for="countries_of_operation_worldwide" class="form-label">Countries of Operation Wordwide
                    *</label>
                <input class="form-check-input" type="checkbox" name="countries_of_operation_worldwide"
                    id="countries_of_operation_worldwide" @if ($agent->countries_of_operation_worldwide == 1) checked
                @endif>
            </div>
